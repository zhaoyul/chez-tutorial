...
;; 3. 记录继承 - 单独定义，不使用 parent
(define-record-type employee
  (fields name age employee-id salary)
  (protocol
   (lambda (new)
     (lambda (name age employee-id salary)
       (unless (string? name)
         (error 'make-employee "name must be a string"))
       (unless (and (integer? age) (>= age 0))
         (error 'make-employee "age must be non-negative integer"))
       (new name age employee-id salary)))))
...
(define (test-records)
  ...
  (let ([acc (make-bank-account "123456" 1000)])
  ...
  ;; 测试 employee 记录
  (display "\n3. Employee 记录:\n")
  (let ([e1 (make-employee "Bob" 35 "E001" 75000)])
  ...
  ;; 测试树
  (display "\n4. 二叉搜索树:\n")
  (let ([tree (fold-left insert-bst #f '(5 3 7 1 9))])
    (display "  中序遍历: ")
